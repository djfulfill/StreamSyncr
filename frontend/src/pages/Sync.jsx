...
export default function Sync() {
  ...
  const toggleBg = async () => {
    if (bgRunning) {
      await syncApi.stopBackground();
      setBgRunning(false);
    } else {
      try {
        await syncApi.startBackground({ ...getConfig(), intervalMinutes: bgInterval });
        setBgRunning(true);
      } catch {}
    }
  };
        ...
        {[{ id: 'sync', label: 'Sync Now', icon: RefreshCw }, { id: 'history', label: 'History', icon: History }].map((t) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${
              tab === t.id ? 'bg-glow/20 text-glow border border-glow/30' : 'text-mist hover:text-snow bg-ghost'
            }`}
          >
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              {connected.map((s) => (
                <span
                  key={s}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-ghost text-sm text-snow"
                >
          ...
          >
                ...
                {STRATEGIES.map((s) => (
                  <button
                    ...
                    className={`p-3 rounded-xl text-left transition-all ${
                      strategy === s.id ? 'bg-glow/20 border border-glow/40 text-snow' : 'bg-ghost border border-transparent text-mist hover:text-snow'
                    }`}
                  >
            ...
            <div>
              ...
              <div className="flex gap-3">
                {[
                  { l: 'Watched', i: Eye, v: syncWatched, set: setSyncWatched },
                  { l: 'Ratings', i: Star, v: syncRatings, set: setSyncRatings },
                  { l: 'Favorites', i: Heart, v: syncFavorites, set: setSyncFavorites },
                ].map((t) => (
                  <button
                    key={t.l}
                    onClick={() => t.set(!t.v)}
                    className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm transition-all ${
                      t.v ? 'bg-glow/20 border border-glow/40 text-snow' : 'bg-ghost border border-transparent text-mist'
                    }`}
                  >
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-mist">Preview changes without writing</p>
              </div>
              <button
                onClick={() => setDryRun(!dryRun)}
                className={`w-12 h-6 rounded-full transition-colors relative ${dryRun ? 'bg-flame' : 'bg-whisper'}`}
              >
                <div
                  className={`w-5 h-5 rounded-full bg-snow absolute top-0.5 transition-transform ${dryRun ? 'translate-x-6' : 'translate-x-0.5'}`}
                />
              </button>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                  bgRunning ? 'bg-ember/20 text-ember border border-ember/30' : 'bg-flame/20 text-flame border border-flame/30'
                }`}
              >
                {bgRunning ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
            ...
            <div className="flex items-center gap-4">
              ...
              <select
                ...
                className="bg-ghost text-snow text-sm px-3 py-2 rounded-xl border-0 outline-none"
              >
              ...
              {bgRunning && (
                <span className="text-xs text-rose flex items-center gap-1">
                  <div className="w-2 h-2 rounded-full bg-flame pulse" />
                  Running
                </span>
              )}
            </div>
              ...
              >
                <div className="glass-strong p-5 glow-glow">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      { i: Film, l: 'Items', v: result.items_synced, c: 'text-glow' },
                      { i: Eye, l: 'Watched', v: result.watched_synced, c: 'text-mint' },
                      { i: Star, l: 'Ratings', v: result.ratings_synced, c: 'text-ember' },
                      { i: Heart, l: 'Favorites', v: result.favorites_synced, c: 'text-rose' },
                    ].map((s) => (
                      <div key={s.l} className="text-center p-3 rounded-xl bg-ghost">
                        <s.i className={`w-4 h-4 ${s.c} mx-auto mb-1`} />
                        <div className="font-display font-bold text-xl text-snow">{s.v}</div>
                        <div className="text-xs text-mist">{s.l}</div>
                      </div>
                    ))}
                  </div>
                </div>
                        ...
                        <div key={i} className="rounded-xl bg-ghost overflow-hidden">
                          ...
                          >
                            <div className="flex-1 min-w-0">
                              <span className="text-sm text-snow font-medium truncate block">{ch.item}</span>
                              <span className="text-xs text-mist">
                                {ch.service} · {ch.field}
                              </span>
                            </div>
                            <div className="flex items-center gap-2 text-sm">
                              <span className="text-mist">{String(ch.old)}</span>
                              <ArrowRight className="w-3 h-3 text-glow" />
                              <span className="text-snow font-medium">{String(ch.new)}</span>
                            </div>
                            {expandedChange === i ? (
                          ...
                          <AnimatePresence>
                            {expandedChange === i && (
                              <motion.div
                                ...
                                className="px-3 pb-3 text-xs text-mist border-t border-whisper pt-2"
                              >
                                IMDb: {ch.imdb_id || '—'}
                              </motion.div>
                            )}
          ...
          <div className="glass-strong p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                <button
                  onClick={loadHistory}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm text-mist hover:text-snow bg-ghost transition-colors"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm text-rose hover:bg-rose/10 bg-ghost transition-colors"
                >
            ...
            {logStats && logStats.total_syncs > 0 && (
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="p-3 rounded-xl bg-ghost text-center">
                  <div className="font-bold text-lg text-snow">{logStats.total_syncs}</div>
                  <div className="text-xs text-mist">Total Syncs</div>
                </div>
                <div className="p-3 rounded-xl bg-ghost text-center">
                  <div className="font-bold text-lg text-snow">{logStats.total_items}</div>
                  <div className="text-xs text-mist">Items Synced</div>
                </div>
                <div className="p-3 rounded-xl bg-ghost text-center">
                  <div className="font-bold text-lg text-rose">{logStats.total_errors}</div>
                  <div className="text-xs text-mist">Errors</div>
                </div>
              </div>
            )}
            {logLoading ? (
              <div className="text-center py-8">
                <Loader2 className="w-6 h-6 text-glow animate-spin mx-auto" />
              </div>
            ) : logEntries.length === 0 ? (
                  ...
                  <div key={i} className="p-3 rounded-xl bg-ghost flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm text-snow">
                        {entry.type === 'sync_error' ? 'Sync failed' : `Synced ${entry.items_synced || 0} items`}
                      </div>
