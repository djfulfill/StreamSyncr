/**
 * StreamSyncr Jackett Provider for Nuvio
 * 
 * Searches torrents via StreamSyncr's Jackett scraper.
 * Requires a self-hosted Jackett instance configured in StreamSyncr.
 */
...
async function getStreams(id, type, title, year, season, episode) {
  try {
    ...
    return (data.streams || []).map((stream) => ({
      ...
      url: stream.url,
      behaviorHints: {
        bingeGroup: `streamsyncr-jackett-${id}`,
      },
    }));
  } catch (error) {
    console.error("StreamSyncr Jackett error:", error);
    return [];
  }
}
