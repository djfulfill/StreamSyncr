/**
 * StreamSyncr MagnetDL Provider for Nuvio
 * 
 * Magnet link aggregator — free, no auth required.
 */
...
async function getStreams(id, type, title, year, season, episode) {
  try {
    ...
    return (data.streams || []).map((stream) => ({
      ...
      url: stream.url,
      behaviorHints: {
        bingeGroup: `streamsyncr-magnetdl-${id}`,
      },
    }));
  } catch (error) {
    console.error("StreamSyncr MagnetDL error:", error);
    return [];
  }
}
